...
func (cmd *WorkspaceCmd) Run(ctx context.Context, stdin io.Reader, stdout io.Writer, stderr io.Writer) error {
	...
	if instanceEnv != "" {
		...
		if instance == nil && terminal.IsTerminalIn {
			instance, err = createInstanceFromForm(ctx, baseClient, workspaceID, workspaceUID, cmd.Log)
			if err != nil {
				return err
			}
		}
	...
	if instance == nil {
		return fmt.Errorf("unable to create instance")
	}
